import React, {  useContext } from "react";
import { useNavigate } from "react-router-dom"; 
import "../styles/Cart.css";
import { CartContext } from "../CartContext";
import { API_URL } from "../App";
import { Helmet } from "react-helmet-async";


export default function Cart() {
    
    const { cart, addProductToCart, priceToFloat,floatToPrice, calculateTotalPrice, totalPrice, decreaseProductQuantity, deleteProductFromCart } = useContext(CartContext);

    
        
    const navigate = useNavigate();


    const handleClick = () => {
        window.scrollTo(0, 0);
        navigate("/finalize");
    }

    
 return (
    <>
        <Helmet>
            <title>Scraps | Koszyk</title>
            
        </Helmet>
        <div className="cart-main">
            <div className="cart-c1">
                {cart.map((produkt, index) => (
                    <div className="cart-c1-product" key={index}>
                        <img src={`${API_URL}${produkt.zdjecia[0]}`}  className="cart-c1-i1" alt="" />
                        <div className="cart-c1-product-text">
                            <p className="cart-produkt-nazwa" onClick={()=>{const toNavigate="/product/" +produkt.id; navigate(toNavigate)}}>{produkt.nazwa}</p>
                            <p>{floatToPrice(produkt.cena)} zł</p>
                            <div className="cart-c1-product-text-c1">
                                <div className="cart-c1-product-text-c1-c1">
                                        <span className="cart-c1-product-text-c1-c1-sign" onClick={() => decreaseProductQuantity(produkt)}>-</span>
                                        <input
                                            min="1"
                                            className="cart-c1-product-text-c1-input"
                                            type="number"
                                            value= {produkt.ilosc}

                                            readOnly
                                        />
                                        <span className="cart-c1-product-text-c1-c1-sign" onClick={() => addProductToCart(produkt)}>+</span>
                                </div>
                                <p>{floatToPrice(produkt.cena * produkt.ilosc)} zł</p>
                            </div>
                        </div>
                        <svg  className="cart-bin" fill="#ffffff"  onClick={() => {deleteProductFromCart(produkt)}} version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 408.483 408.483" xmlSpace="preserve" stroke="#ffffff"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M87.748,388.784c0.461,11.01,9.521,19.699,20.539,19.699h191.911c11.018,0,20.078-8.689,20.539-19.699l13.705-289.316 H74.043L87.748,388.784z M247.655,171.329c0-4.61,3.738-8.349,8.35-8.349h13.355c4.609,0,8.35,3.738,8.35,8.349v165.293 c0,4.611-3.738,8.349-8.35,8.349h-13.355c-4.61,0-8.35-3.736-8.35-8.349V171.329z M189.216,171.329 c0-4.61,3.738-8.349,8.349-8.349h13.355c4.609,0,8.349,3.738,8.349,8.349v165.293c0,4.611-3.737,8.349-8.349,8.349h-13.355 c-4.61,0-8.349-3.736-8.349-8.349V171.329L189.216,171.329z M130.775,171.329c0-4.61,3.738-8.349,8.349-8.349h13.356 c4.61,0,8.349,3.738,8.349,8.349v165.293c0,4.611-3.738,8.349-8.349,8.349h-13.356c-4.61,0-8.349-3.736-8.349-8.349V171.329z"></path> <path d="M343.567,21.043h-88.535V4.305c0-2.377-1.927-4.305-4.305-4.305h-92.971c-2.377,0-4.304,1.928-4.304,4.305v16.737H64.916 c-7.125,0-12.9,5.776-12.9,12.901V74.47h304.451V33.944C356.467,26.819,350.692,21.043,343.567,21.043z"></path> </g> </g> </g></svg>
                    </div>
                ))}
            </div>
            <div className="cart-c2">
                        <h1 className="cart-c2-h2">Podsumowanie zamówienia</h1>
                        <p className="cart-c2-p">Razem {floatToPrice(totalPrice)} zł</p> 
                        <button className="cart-c2-button" onClick={handleClick}>Zamów</button>
            </div>
        </div>
    </>
);
}

